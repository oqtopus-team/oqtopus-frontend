...
import {
  PauliGateType,
  WasmOneControlOneTargetGateData,
  WasmOneQubitGateData,
  WasmOneQubitRotationGateData,
  WasmPauliGateData,
  WasmQuantumGateData,
...
export const convertCircuit = (
  circuit: QuantumCircuit,
  qubitNumber: number
): QulacsWasmClientType.CircuitInfo => {
  const maxDepth = circuit[0]?.length ?? 0;
  const circuitSteps = [...new Array(maxDepth)].map((_, s) => {
...
export const convertComposerGate = (gate: ComposerGate): WasmQuantumGateData[] => {
  switch (gate._tag) {
    ...
    case 'multiRowGateEmptyBlock':
      return [['i', gate.row]];
    case '$custom_gate':
      throw new Error('Custom gates are not supported.');
  }
