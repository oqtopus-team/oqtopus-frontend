...
import {
  ...
  ParametricExpectationValuePoint,
} from './LocalSimlParametricExpectationValueChart';
...
export const ExpectationValueEstimation: React.FC<Props> = (props) => {
  ...
  const parametricExpectationValueData = useMemo(() => {
    return props.parametricExpectationValue.expectationValueMap.map(
      (point): ParametricExpectationValuePoint => {
        return {
          param: point.param,
  ...
  return (
    <div className={clsx(['flex', 'items-center', 'gap-5'])}>
      <div className={clsx([['border', 'border-chart-border', 'rounded-lg', 'p-3']])}>
        <div className={clsx([['text-sm', 'text-chart-title'], ['pt-1', 'pb-3'], ['w-full']])}>
        ...
        >
          ...
          >
            <div className={clsx([['font-bold'], ['text-sm', 'text-neutral-content']])}>
              {t('composer.control_panel.siml.observable')}
            </div>
            <div className={clsx([['bg-gray-bg'], ['p-5', 'w-full', 'overflow-x-auto']])}>
      ...
      <div className={clsx([['border', 'border-chart-border', 'rounded-lg', 'p-3']])}>
        ...
        >
          <ParametricExpectationValueChart data={parametricExpectationValueData} />
        </div>
      </div>
    </div>
  );
