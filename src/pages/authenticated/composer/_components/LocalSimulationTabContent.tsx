...
export const LocalSimulationTabContent: React.FC<Props> = (props) => {
  ...
  const [parametricExpectationValue, setParametricExpectationValue] = useState(
    emptyParametricExpectedValueResult
  );
    ...
    if (program.startsWith('//')) {
      ...
      try {
        ...
        const observable = ((
          ...
          if (jobType == 'estimation' && operator !== undefined && !isZeroOperator(operator)) {
            return operator;
          }
        ...
        if (props.jobType === 'estimation' && props.selectedParametricGatePosition !== undefined) {
          const newParametricExpectationValue = qulacsApi.requestParametricExpectedValue(
            circuitInfo,
            observableInfo,
            51,
        ...
        <>
          ...
          <div>
            <div className={clsx(['flex', 'items-center', 'gap-3', 'my-3'])}>
              <span className={clsx('text-nowrap', 'p-3')}>
            ...
            <SamplingExecutionResult qubitNumber={props.qubitNumber} shotResult={shotResult} />
          </div>
        </>
      ) : null}
