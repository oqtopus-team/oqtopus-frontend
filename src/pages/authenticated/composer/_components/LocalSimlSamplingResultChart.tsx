    ...
    return (
      <div className="m-0 p-2.5 bg-primary-content border border-neutral-content whitespace-nowrap shadow-sm rounded">
        ...
        <div className="text-gray-600 text-sm mt-1 space-y-0.5">
          {data.count != null && <p className="m-0 text-[#3f51b5]">Count: {data.count}</p>}
        </div>
      </div>
    );
  }
...
export const SimulatorResultSamplingChart: React.FC<Props> = ({ data, shot }) => {
  ...
  return (
    <div className={clsx([['border', 'border-chart-border', 'rounded-lg'], ['p-3']])}>
      ...
      <div className={clsx(['m-2'])} style={{ width: '400px', height: '320px' }}>
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={data}>
            ...
            <YAxis yAxisId="1" domain={[0, shot]} scale="auto">
              <Label value="counts" angle={-90} position="insideBottomLeft" />
            </YAxis>
            <Tooltip
            ...
            />
            <Bar dataKey="count" yAxisId="1" fill="#82ca9d" fillOpacity="0.4" />
            <Line type="linear" dataKey="probabilityVector" yAxisId="1" stroke="red" dot={false} />
          </ComposedChart>
        </ResponsiveContainer>
      </div>
    </div>
