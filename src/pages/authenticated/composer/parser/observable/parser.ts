...
const tokenSuchThat = (prd: (tok: Token) => boolean): ObservableParser<SourceToken> =>
  Parser((st) => {
    const [lexRes, nextState] = lex(st.src, st.pos);
    switch (lexRes._tag) {
      case 'ParseOk':
        if (prd(lexRes.value.it)) {
          return [ParseOk(lexRes.value), { pos: nextState.pos, src: nextState.input }];
        } else {
          return [
            ParseFail(
              mkRangedError(
      ...
      case 'ParseFail':
        return [
          ParseFail(
            mkRangedError(lexRes.value[0], mkSourceRange(lexRes.value[1], lexRes.value[1]))
          ),
          { pos: nextState.pos, src: nextState.input },
        ];
    }
...
const reject = <T>(toks: SourceToken[], err?: ObservableParserError): ObservableParser<T> =>
  Parser.fail(
    mkRangedError(
...
export const parseIndexedPauliGate: ObservableParser<[PauliGateType, number]> = parsePauliGate.then(
  (gatename) => parseInt.then((n) => Parser.pure([gatename, n]))
);

/**
 * The parsed result is an array of Pauli matrix type, sorted according to the qubit index.
 */
export const parsePauliString: ObservableParser<PauliGateType[]> = P.many(
  parseIndexedPauliGate
).map((gates) => {
  const arranged: Record<number, PauliGateType> = gates.reduce(
    (acc, [gate, i]) => ({ ...acc, [i]: gate }),
    {}
  );
  return Array.from(
  ...
  const reportError = <T>(res: ParseResult<RangedError, unknown>): ParseResult<string, T> => {
    switch (res._tag) {
      ...
      case 'ParseFail':
        ...
        const errorMsg =
          'An error has occurred during parsing operator:\n' +
