...
const lexFailWith = <T>(
  e: ObservableParserError,
  s: LexerState
): [ParseResult<PositionedError, T>, LexerState] => [ParseFail(mkPositionedError(e, s.pos)), s];
const lexOk = <T>(t: T, s: LexerState): [ParseResult<PositionedError, T>, LexerState] => [
  ParseOk(t),
  s,
];
...
const regex = (re: RegExp): PauliStringLexer<SourcePhrase<string>> =>
  Parser(({ input, pos }) => {
    ...
    return lexOk(mkSourcePhrase(matched[0], mkSourceRange(pos, endPos)), {
      input: nextInp,
      pos: endPos,
    });
  });
...
const tokGateChar: PauliStringLexer<SourceToken> = regex(/[a-zA-Z]/).map(({ it: ch, at }) =>
  mkSourcePhrase(Token.TokChar(ch), at)
);
...
export const tokenizer = whitespaces.then((_) => oneOf(tokInt, tokGateChar, eof));

export const lex = (src: string, offset: SourcePos) =>
  tokenizer.runParser({ input: src, pos: offset });
