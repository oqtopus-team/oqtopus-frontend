...
export type ObservableParserError = Unexpected | InvalidGate | QubitIndexTooLarge | UnexpectedToken;
...
export const mkPositionedError = (e: ObservableParserError, p: SourcePos): PositionedError => [
  e,
  p,
];
...
export const renderParseError = (e: ObservableParserError): string => {
  switch (e[0]) {
    case 'InvalidGate':
      return `Invalid gate type: ${e[1]}.`;
    case 'QubitIndexTooLarge':
      return `Qubit index ${e[1]} is too big.`;
    case 'Uexpected':
      return `Unexpected input: ${e[1]}`;
    case 'UnexpectedToken':
      return `Unexpected token: ${printToken(e[1])}`;
  }
};
